...
- Каждый источник фактического L3 direct snapshot получает descriptor `READY/STALE/BUILDING/FAIL_CLOSED`; незатенённого direct-вклада нет. Осознанное отличие: production DDA отключён, потому что достижимый 96-шаговый traversal резко раздувал обычный Metal fragment PSO; непокрытый L5 источник остаётся fail-closed до готовой страницы.
- Исправлены dense-lava self-shadow и квадратные кольца на плоских receiver: footprint входит в cache key, а cached hit сравнивается с tangent plane receiver. Релевантные block changes обновляют cache асинхронно; camera origin сам по себе страницу не инвалидирует.
...
- Heap payload страницы копируется прямо в mapped Metal staging; это устранило воспроизводимый SIGSEGV на `64²` upload. `clean check` прошёл (`64` задачи), включая actual GLSL→SPIR-V→MSL, CPU/numeric/native ABI и Metal Validation; отдельный live Metal API + Shader Validation завершился без crash/fault и fallback после admission.
- M1 Pro, Balanced HDR Native, accepted `1800 + 3000` кадров: `70.43 FPS`, GPU p95 `20.21 ms`, минимум окна `68.62 FPS`, `0` dropped events. Установившееся покрытие: `594/594` descriptors, `46 READY`, `548 FAIL_CLOSED`, `DDA=0`, очереди/capacity-blocked/unshadowed contribution — `0`.
- Автоматические contracts покрывают variable cube pages, fence-safe residency, slab/fence/transmittance, dense-lava footprint и receiver-plane seam fix. Финальный ручной live-signoff лавы, sea lantern и мелких контуров после перезапуска остаётся обязательным: Computer Use и новые screenshots не применялись.
