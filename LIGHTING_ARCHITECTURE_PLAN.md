...
- ✅ Дополнение L6 — сглаживание pixelated shadows выполнено: страницы `8²/16²` используют непрерывную seam-safe триангуляцию с `1–3` taps, а детальные `32²/64²` сохраняют точную одновыборочную тень. Числовой контракт покрывает внутренние границы, `12` cubemap seams и `8` углов; `clean check` (`64` задачи), actual GLSL→SPIR-V→MSL и `300+300` Metal API/GPU Validation прошли. Финальный accepted `1800+3000` дал `66.72 FPS`, GPU p95 `21.39 ms`, минимум окна `66.36 FPS`, `0` dropped/FAIL при `46 READY / 548 APPROXIMATE / 0 FAIL_CLOSED`.
