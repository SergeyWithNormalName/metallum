...
- ✅ Дополнение L4: резкие рывки солнечных теней были не tick-only временем Minecraft, а дискретным rebasing вращающейся CSM-сетки и переключением light basis. Каскады теперь используют непрерывный fixed-Z celestial basis и camera-local quarter-texel anchor; новых pass/resources/fetch нет. Регресс-тест до исправления воспроизводил скачок `1.03` texel, `clean check` и два accepted 3000-frame run прошли (`75.09/76.06 FPS`, repeat GPU p95 `18.66 ms` против прежних `19.08 ms`, `0` dropped/FAIL/fallback); единичный present tail сохранил строгий compare красным, поэтому плавность при живом ходе солнца и обычном/диагональном движении остаётся обязательным ручным signoff.
