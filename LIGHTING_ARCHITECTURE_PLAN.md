...
- Исправлены dense-lava self-shadow и квадратные кольца на плоских receiver: footprint входит в cache key, а cached hit сравнивается с tangent plane receiver. При релевантном block change старая страница остаётся `STALE` и продолжает освещать кадр до атомарного `reserve → blit → commit`; same-topology mirror публикуется после каждого принятого L5 batch, а дешёвый `8²` refresh идёт в отдельной срочной очереди. Camera origin сам по себе страницу не инвалидирует.
