...
- ✅ Follow-up L6 — seam-safe full-cell `4-tap` visibility теперь применяется к каждому `READY/STALE` local-light contribution внутри уже ограниченного fragment loop, а не только к самому яркому источнику. Все resident L6-тени получают одинаково плавный край на страницах `8²–64²`, включая стыки cubemap; `APPROXIMATE_DIRECT`, partial-receiver guard, fail-open descriptor policy и предел `256` кандидатов не изменены. Производительность этого quality-first исправления требует отдельного сопоставимого live A/B.
...
- Дневной блик солнца на воде использует мягкую квадратную маску в tangent plane отражения:
  это сохраняет пиксельный силуэт ванильного солнца, но не превращает его в мерцающую жёсткую
  плитку. Маска применяется и к analytic celestial reflection, и к solar GGX только для water;
  луна, local lights и остальные материалы сохраняют прежнюю круглую физическую лобу. Новых
  ресурсов, проходов, ABI или работы на non-water path нет; `clean check` прошёл, а финальный
  визуальный signoff на открытой воде остаётся ручным.
