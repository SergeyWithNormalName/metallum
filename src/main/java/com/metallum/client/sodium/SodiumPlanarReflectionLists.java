...
/** Exact Sodium 0.9.1 opaque-section list for the reflected camera's clipped frustum. */
public final class SodiumPlanarReflectionLists implements ChunkRenderListIterable {
    private static long activeToken;
    private static SodiumPlanarReflectionLists activeLists;
    private final List<ChunkRenderList> lists;

    private SodiumPlanarReflectionLists(final List<ChunkRenderList> lists) {
    ...
    public static ChunkRenderListIterable select(
        ...
        Matrix4fc clipFromWorldRelative = Objects.requireNonNull(
                PlanarReflectionRenderer.activeTerrainVisibilityProjection(),
                "active planar-reflection visibility projection"
        );
        CameraPosition camera = Objects.requireNonNull(
                PlanarReflectionRenderer.activeCameraPosition(),
                "active planar-reflection camera position"
        );
        activeToken = token;
        PlanarReflectionRenderer.registerTerrainCleanup(SodiumPlanarReflectionLists::release);
        activeLists = build(regions, clipFromWorldRelative, camera, token);
        return activeLists;
    }
    ...
    private static SodiumPlanarReflectionLists build(
        ...
        for (RenderRegion region : loaded) {
            if (region.getResources() == null || !intersectsRegion(clipFromWorldRelative, camera, region)) {
                continue;
            }
            ...
            for (int index = 0; index < RenderRegion.REGION_SIZE; index++) {
                if ((region.getSectionFlags(index) & RenderSectionFlags.MASK_HAS_BLOCK_GEOMETRY) == 0) {
                    continue;
                }
            ...
            if (list.getSectionsWithGeometryCount() > 0) {
                ((SodiumPlanarReflectionBatchAccess) region).metallum$preparePlanarReflectionBatch(
                        token, sections0, sections1, sections2, sections3
                );
                lists.add(list);
            }
        }
        return new SodiumPlanarReflectionLists(lists);
    }
    ...
    public Iterator<ChunkRenderList> iterator(final boolean reverse) {
        ...
        return new Iterator<>() {
            private int index = SodiumPlanarReflectionLists.this.lists.size();
            ...
            public ChunkRenderList next() {
                ...
                return SodiumPlanarReflectionLists.this.lists.get(--this.index);
            }
        };
    }
}
