...
/** Carries held block emission through the otherwise legacy item lightmap coordinates. */
public final class HeldItemEmission {
    ...
    public static int surfaceEmission(
        ...
        if (!isHand(displayContext) || !(item instanceof BlockItem blockItem)) {
            return 0;
        }
    ...
    static int surfaceEmission(
        ...
        return isHand(displayContext) ? Math.clamp(blockEmission, 0, MAX_EMISSION) : 0;
    }
    ...
    private static boolean isHand(final ItemDisplayContext displayContext) {
