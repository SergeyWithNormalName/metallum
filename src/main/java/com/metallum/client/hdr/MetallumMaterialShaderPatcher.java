...
public final class MetallumMaterialShaderPatcher {
    ...
    private static Result patchSodium(final Stage stage, final String source) {
        ...
        String fogReplacement = "    uint metallumEmissionCode = (metallumMaterial >> 3u) & 15u;\n"
                ...
                + "        float metallumDistance = max(v_FragDistance.x, v_FragDistance.y);\n        float metallumExtinction = clamp(1.0 - max(metallumDistance - 16.0, 0.0) / 96.0, 0.0, 1.0);\n"
                + "        float metallumHdrExcess = max(metallumEmission - 1.0, 0.0);\n"
                + "        float metallumBaseEmission = min(metallumEmission, 1.0);\n"
                + "        if (metallumEmissionCode < 10u) {\n"
                + "            metallumBaseEmission *= mix(0.4, 1.0, metallumExtinction);\n"
                + "        }\n"
                + "        vec3 metallumAuthoredRadiance = metallumUnlitBase\n"
                + "                * (metallumBaseEmission + metallumHdrExcess * metallumExtinction);\n"
