...
- **Production path**: Fixed Temporal presets use their render-sized inputs. Temporal Dynamic is hybrid: it uses native/Spatial from 100% through 70%, switches into MetalFX Temporal at the next DRS step (65%), and returns to Spatial at `>=75%` (hysteresis avoids history churn). Temporal Dynamic may lower DRS to 45%; Spatial/native retains its 50% floor. While Temporal is active it uses MetalFX's `inputContent*` API: one display-sized scaler, color/depth inputs and output, with GPU blits packing only the active low-resolution world rectangle at the origin. The triple-buffered `RG16Float` motion and `R8Unorm` reactive ring is likewise display-sized but rasterized only inside that active rectangle.
