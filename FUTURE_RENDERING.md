...
- **Production path**: Fixed Temporal presets use their render-sized inputs. Temporal Dynamic is a bounded three-stage path: Native at 100%; Spatial from 95% down to 60% (fast proportional downscale, 5%-step recovery after 12 fresh low-GPU samples); then Temporal from 55% down to 50%. After Spatial has remained at 60%, 45 consecutive completed GPU samples above 16.5 ms enter Temporal at 55%. After Temporal has reached 60%, 60 consecutive samples below 13.0 ms return to Spatial at 70%. Dynamic Temporal uses the established Quality mip-bias sampler for a stable 50-60% input. While Temporal is active it uses MetalFX's `inputContent*` API: one display-sized scaler, color/depth inputs and output, with GPU blits packing only the active low-resolution world rectangle at the origin. The triple-buffered `RG16Float` motion and `R8Unorm` reactive ring is likewise display-sized but rasterized only inside that active rectangle.
