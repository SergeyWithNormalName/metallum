...
- **Safety contract**: Format, usage, device, active-content and generation checks fail closed to native resolution. Inside an already active Dynamic-Temporal session, the physical scaler, motion/reactive ring, staging depth and depth history are display-sized, GPU-private and reused across input-content changes; every render-extent or renderer-generation change resets history. A Native ↔ Temporal policy transition is broader than an input-content change: it changes the renderer feature generation and may create or release Temporal resources. It is therefore correct and safe, but is not a zero-allocation promise.
...
- **Boundary and pacing**: Minecraft still physically resizes world targets to reduce shaded pixels. The Native ↔ Temporal switch also changes the renderer feature generation: it can allocate a Temporal motion/reactive ring and native workspace on entry, and clears them on return to Native. Private resource destruction is deferred behind in-flight work and MetalFX optimized compilation is asynchronous, but neither fact makes the transition allocation-free or guarantees a zero-ms hitch. The policy also uses Temporal frame time as a proxy for whether Native is affordable; unusual workloads can therefore alternate between modes. The authoritative current audit and measurement plan are in [docs/TEMPORAL_UPSCALING_DRS.md](docs/TEMPORAL_UPSCALING_DRS.md).
